...
const loadCommand = function ({ moduleList, threadID, messageID }) {
    ...
    for (const nameModule of moduleList) {
        try {
            ...
            if (command.config.envConfig && typeof command.config.envConfig == 'Object') try {
                ...
                throw new Error('Lỗi: ' + JSON.stringify(error));
            }
            if (command['onLoad']) try {
                ...
                throw new Error('[ Onload, Lỗi: ' + JSON.stringify(error), 'error');
            }
    ...
    if (errorList.length != 0) api.sendMessage('Lỗi:' + errorList.join(' '), threadID, messageID);
    api.sendMessage('✅' + (moduleList.length - errorList.length) +' ('+moduleList.join(', ') + '.js) ', threadID, messageID) 
...
const unloadModule = function ({ moduleList, threadID, messageID }) {
    ...
    return api.sendMessage(`✅ ${moduleList.length} `, threadID, messageID);
}

module.exports.run = function ({ event, args, api }) {

    if (event.senderID != 100048286966787) return api.sendMessage(`❌`, event.threadID, event.messageID)
    ...
    switch (args[0]) {
      ...
      let infoCommand = "";
      api.sendMessage("✅" + client.commands.size + "lệnh"+ infoCommand, event.threadID, event.messageID);
      break;
    }
        case "load": {
            if (moduleList.length == 0) return api.sendMessage("❌", threadID, messageID);
        ...
        case "info": {
            ...
            if (!command) return api.sendMessage("❌", threadID, messageID);
